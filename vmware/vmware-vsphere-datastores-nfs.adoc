...
* 在 ONTAP 集群中每个节点上的每个 SVM 使用单个逻辑接口 (LIF)。过去推荐的每个数据存储区一个 LIF 不再是必要的。虽然直接访问（同一节点上的 LIF 和数据存储区）是最好的，但不要担心间接访问，因为性能影响通常很小（微秒）。
...
[TIP]
====
* nconnect 在 vSphere 8 中的最大值为 4，默认值为 1。vSphere 中的最大值限制可以通过高级设置在每个主机基础上提高，尽管通常不需要这样做。
* 对于需要比单个TCP连接所能提供的性能更高的环境、建议将值设置为4。
* 请注意，ESXi 限制为 256 个 NFS 连接，每个 nconnect 连接都计入该总数。例如，nconnect=4 的两个数据存储区将计为总共八个连接。
* 在生产环境中实施大规模更改之前，请务必测试 nconnect 对环境的性能影响。
...
* 如 link:vmware-vsphere-settings.html["设置"] 中所述，如果您不将 vSphere CSI 用于 Kubernetes，则应按  https://knowledge.broadcom.com/external/article/386364/reducing-excessive-vsan-cnssync-warnings.html["VMware 知识库 386364"^] 设置 newSyncInterval
...
* 虽然 ONTAP 提供了一个灵活的卷命名空间结构，可以使用连接点在树中排列卷，但这种方法对 vSphere 没有价值。无论存储的命名空间层次结构如何，它都会在数据存储的根目录下为每个 VM 创建一个目录。因此，最佳做法是简单地在 SVM 的根卷上挂载 vSphere 卷的连接路径，这就是 ONTAP tools for VMware vSphere 配置数据存储的方式。没有嵌套的连接路径也意味着没有卷依赖于根卷以外的任何卷，并且使卷离线或销毁卷（即使是有意销毁）不会影响到其他卷的路径。
