...
vSphere 包括对存储设备的多个路径的内置支持。vSphere 可以检测受支持的存储系统的存储设备类型，并自动配置多路径堆栈以支持正在使用的存储系统的功能，无论使用哪种协议，或者是否使用 ASA、AFF、FAS 或软件定义的 ONTAP。
...
NOTE: 有关 ESXi 中最新支持限制的信息，请参见link:https://configmax.broadcom.com/guest?vmwareproduct=vSphere&release=vSphere%208.0&categories=2-0["VMware配置最大值工具"^]。
...
* 对于 iSCSI 或 NVMe/TCP 网络，当存在多个虚拟交换机时，请在不同的未路由网络子网上使用多个 VMkernel 网络接口，并使用 NIC 组。您还可以使用连接到多个物理交换机的多个物理 NIC 来提供 HA 和增加的吞吐量。在 ONTAP 中，配置单模接口组进行故障转移，其中两个或多个链路连接到两个或多个交换机，或者使用 LACP 或其他具有多模接口组的链路聚合技术来提供 HA 和链路聚合的好处。请记住，必须在每个 VMkernel 端口和每个物理接口上启用 NVMe/TCP，并且必须对它们进行对齐，以便 NVMe/TCP VMkernel 端口可以访问启用了 NVMe/TCP 的物理 NIC。下图说明了基于 IP 的 SAN 协议的多路径连接。image:vsphere_ontap_image2.png["多路径连接"]
