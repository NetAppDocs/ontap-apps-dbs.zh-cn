...
虚拟化管理员有时更愿意通过从配置中删除网络交换机来简化其基础架构。这样可以创建一个更加独立的解决方案，所需组件更少，占用的机架空间也更少。根据所使用的协议，存在一些重要限制。
...
从 ONTAP 9.19.1 开始，使用特定存储系统 FC 适配器的传统 FC-SAN (SCSI) 和 NVMe/FC 均支持直接连接 FC。HBA 将建立标准点对点 FC 连接。在 ONTAP 9.19.1 之前，ONTAP 存储系统需要 NPIV，这意味着存储系统必须连接到也支持 NPIV 的 FC 交换机。
