...
聚合是 AFF 存储配置的最低级别存储容器。互联网上存在一些旧文档，建议将 IO 分离到不同的底层驱动器集上。不建议在 ONTAP 中采用此方法。NetApp 已使用数据文件和事务日志文件分离的共享和专用聚合执行了各种 I/O 工作负载表征测试。测试表明，一个具有更多 RAID 组和驱动器的大型聚合可以优化和提高存储性能，并且管理员更容易管理，原因有以下两点：

* 一个大型聚合/SA使所有驱动器的I/O功能可供所有文件使用。
* 一个大型聚合可以最高效地利用磁盘空间。
