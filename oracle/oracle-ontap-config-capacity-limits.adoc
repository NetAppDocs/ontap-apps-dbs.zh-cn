...
可用空间是指不用于实际数据的任何空间，其中包括聚合本身上未分配的空间以及成分卷中未使用的空间。还必须考虑精简配置。例如，AFF 卷可能包含 1TB LUN，其中只有 50% 被真实数据利用。在精简配置环境中，这将正确地显示为占用 500GB 的空间。但是，在完全配置的环境中，1TB 的全部容量似乎都在使用中。500GB 的未分配空间将被隐藏。实际数据并未使用这一空间，因此在计算总可用空间时应将其计算在内。
