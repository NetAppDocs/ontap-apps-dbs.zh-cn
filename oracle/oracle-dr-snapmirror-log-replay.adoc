...
最重要的要求是将数据文件隔离到一个或多个专用容器中。这可以通过几种方式完成，包括将所有数据文件放置在单个 LUN/namespace、单个 AFF NFS 卷内或放置在多卷或多 LUN/namespace ONTAP 一致性组中。关键是数据文件不得受到任何其他文件类型的影响。原因是为了确保数据文件复制完全独立于其他数据类型（如存档日志）的复制。有关文件布局的其他信息以及有关确保存储布局对 Snapshot 友好的重要详细信息，请参阅link:oracle-dp-online-backup.html#data-layout["数据布局"]。
