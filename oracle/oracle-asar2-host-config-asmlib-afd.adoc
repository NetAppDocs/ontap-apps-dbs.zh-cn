...
summary: 在ASA r2 上使用 Linux 和 ASMlib/ASM 过滤器驱动程序的 Oracle 数据库 
searchtitle: 在ASA r2 上使用 ASMLib/AFD（ASM 过滤器驱动程序）的 Oracle 数据库 
---
...
[role="lead"]
使用 AFD 和 ASMlib 的ASA r2 ONTAP的 Linux 操作系统特有的配置主题。



== ASMlib块大小

ASMlib 是一个可选的 ASM 管理库和相关实用程序。它的主要价值在于能够将 LUN 标记为 ASM 资源，并带有人类可读的标签。
